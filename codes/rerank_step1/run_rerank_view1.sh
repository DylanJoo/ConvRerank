...
MODEL_SIZE=base
CKPT=1100000
INPUT_FILE=convrerank/rerank_step1/input/cast20.canard.train.view1.rerank.txt
TARGET_FILE=convrerank/true.txt
OUTPUT_FILE=convrerank/rerank_step1/output/cast20.canard.train.view1.rerank.txt


echo "Input: ${INPUT_FILE}\t Output: ${OUTPUT_FILE}" >> $YOUR_TPU.out
for ITER in {051..105}; do
  echo "Running iter: $ITER" >> $YOUR_TPU.out
  t5_mesh_transformer \
    --tpu="${YOUR_TPU}" \
